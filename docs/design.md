...
- **OFFSET4種は項目単位フォールバック**：`POINT` 系の節単位切替（4項目全部必須）を流用すると、1項目だけ変えたいときに4項目全部書く必要が生じる。コメントの要求（MYLISTだけ変えたい）に素直に対応するため、節内に対応要素があれば節内値・なければ共通を使う。既存XML（節内OFFSETなし）は無改修で従来動作のままになる。`Load`／`Save` は `Config` 経由のため改修不要で、タブ切替時の既存 `Load`・`Save` がそのままモード別表示・保存になる
