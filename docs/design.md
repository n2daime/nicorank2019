...
- **SPの欠落は案B（あるものを使う）**: `SpMovieInfoFallback` がタイトル空欄分だけ `LastResult` 最新タイトルと `LogOfficial` 期間内初見日（`MIN(集計日)`）で補う。新規取得はしない。初見日は本物の投稿日ではないが、新着救済（基準-7日）の判断には使える。どちらもなければ空のまま残す。案A（スナップショットDBへの文字情報追加）は約900万行の肥大化と取得負荷のため見送り、Issue #41 に分離する。ジャンル空欄は許容し、影響が大きければ案Aを再検討する
