...
- **読み側はRanking優先・SoHistoryは補い**: `CheckSoMovieNeedSabun` は `Ranking` ヒットなしのときだけ `SoHistory` を見る2クエリ逐次。表なし旧DBでは新着扱いで正常終了し、DB異常扱いにしない（新着動画を誤除外しないため）。結合・VIEWの1本化は見送り（共通ケースで速くもならず読みにくくなるため）
