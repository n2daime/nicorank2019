...
- **SoHistoryはID＋4数値＋集計日**: 差分計算に使う分のみ。タグは差分に使わず `FavoriteTagReader` が別経路で取るため含めない。ID主キーで最新1件のみ
- **読み側はRanking優先・SoHistoryは補い**: `CheckSoMovieNeedSabun` は `Ranking` ヒットなしのときだけ `SoHistory` を見る。表なし旧DBでは新着扱いで正常終了し、DB異常扱いにしない（新着動画を誤除外しないため）
- **Ver1移行は日付区切り確定**: 初回の大量削除を一括トランザクションにせず集計日ごとに確定する（一括はWAL肥大で破損の前例あり）。`SoHistory` 移行は置換系で冪等、削除とDROPは存在確認系でやり直し可能。バージョン記録は全工程の成功後に行う。VACUUMは確定後の1回のみ
- **日次は同日トランザクションに同梱・VACUUMなし**: 日次単位は維持し、当日分更新＋`SoHistory` 足し替え＋古い分削除を同じ区切りにする。毎日の最適化は重いため将来の手動最適化（#32）に委ねる
