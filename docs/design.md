...
- **辞書式ではなく数値認識（種別→数字）**: 単純Ordinalでは桁違いのID順が数値順と一致しない（sm199 が sm20 より先になる）。IDの大小を割り当て順と直感的に一致させるため、`RankingIdComparer`（`nicorankLib/Analyze/model`）で種別→数字の順に比べる。数字化不能は全体の辞書式にフォールバックし、最終段も辞書式のため常に決定的になる
