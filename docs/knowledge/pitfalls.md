...
### 7. スナップショット取得の 1000 再生足切り（自前の運用判断）

- **1年超のデータは累積 1000 再生未満を除外**（`flgLimit1000` で URL 切替）。ニコ動側の仕様ではなく v20240806 での自前判断（由来は項目 21）
...
- 本ツールの DB は 2019 年以前の情報を持たないため、以前は nicochart.jp の TSV で過去数字を補完していた
- nicochart.jp が利用できなくなったため補完機能を廃止（#23・2026-08）。代替として、過去ログに差分が取れない so 動画は **ID 番号で判定**する: so40000000 未満は新着偽造（再公開）として集計対象外、so40000000 以上は新着として扱う
- so ID の数字部分は一般動画（sm 等）と共通の採番とみなしている。ID の割り当てルールが変わった場合は閾値を見直す
- 事実（2019-06〜2026-09-07時点）: LogOfficial の起点は 2019 年 6 月で、その頃の so は 3400 万台。以降も予約公開の多い so は同日投稿でも ID 幅が 200 万超に広がり、当時は ID 足切りが誤検知多発で使えず、2007 年〜蓄積のニコチャート TSV に依存した。2026-09-07 時点で so は 4600 万台まで進行（7 年で +1000 万超）、4000 万台の割当ては 2022 年頃
- 判断（2026-08-31 #23）: TSV 遮断を機に自前蓄積＋マージンで脱却可能と判断し ID 判定に切替え。3900 万・4100 万台でも成立するが切りの良さで 4000 万を選択。2024-06 攻撃で予約は全取消のため長期予約の逆転リスクは考慮外とし、誤判定は基本ない想定
- 運用（聞き取り 2026-09-07・#33）: 閾値は上げる予定なし（LogOfficial に 2019 年 6 月以前のデータがない、という前提は時間が経っても変わらないため）。ID 割当ルール自体が変わった場合のみ見直す
- 事実（2019〜2026-09-07時点）: so 再公開は投稿日のみ更新・再生/コメント/マイリス/いいねは引き継がれる。最初に発覚した事例は見るタイツ 1 話（当時 2019/5/18→2019/7/xx に更新、現時点では元の投稿日に訂正済み）。ID は基本シーケンスだが、予約公開（投稿時に ID 割当て・公開日は未来日のため ID 順と日時順が逆転する）があり、so 再公開も予約公開と同じ仕組み（初回 ID のまま投稿日だけ更新）で起きる。審査非公開の ID は再利用されず欠番になる。キリ番祭りはユーザー動画（sm/ss）の話
- 実測（2026-09-07調査・読取り専用）: so 全約 1000 万行走査で同一 ID 間隔 365 日超のギャップ持ち約 5.6 万件（so 全体約 28 万件の約 20％）、イベント累計約 7 万件、直近 1 年再浮上約 2.2 万件（日平均約 60 件）、最大約 2600 日。厳密な発火条件（基準日より新しい投稿日＋再生 1000 超）での絞り込み前母数という留保付き
- 判断（#23・2026-08／聞き取り 2026-09-07・#33）: 更新条件は不明のまま追わない（ニコ動側の場当たり対応のため）。非公開の公式動画が無料再公開されると万単位の再生が付くことは珍しくなく、上位に紛れ込むと週刊ランキングの信頼に関わるため、差分が取れずポイント・各種数値が異常になるリスク回避を優先して除外する。毎回の遭遇頻度は #23 の除外対応済みのため不明だが、母数規模から無視不可
...
### 18. 2026-09-04より前のスナップショットDBは全期間1000再生以上のみ（Issue #30切り分け・2026-09）

- **症状**: タグ検索のヒット件数（例：900件）に対して集計対象が大幅に少ない（例：90件）。`SELECT COUNT(*) FROM Ranking WHERE 再生数 < 1000;` が0件
- **原因**: 2024-08-30〜2026-09-04の取得コード不具合で `flgLimit1000` を無視し常時1000制限URLを使っていた（Issue #19で修正）。件数取得だけ無制限・実取得は1000制限のため、エラーなく低再生動画が欠落する。直近1年も例外ではない
- **対策**: 現行コードでDBを作り直す。目安は上記クエリが0件でないこと（正常DBでは低再生行が大量にある）。2026-09-04より前のDBは総合・SP集計には支障ないがタグ検索には不十分
- **件数不一致の切り分け順**: 1. 1000再生フィルタ（1年超の古動画は仕様で1000以上のみ）2. DBの鮮度（作成日より後の投稿は未収録）3. DB作成の中断（前回比でファイルサイズが著しく小さい場合は再取得）

### 19. 集計スレッド（Task.Run内）からUIコントロールに触らない（Issue #30・2026-09）

- **症状**: `ランキング計算` 押下で「有効ではないスレッド間の操作: コントロールが作成されたスレッド以外のスレッドからコントロール 'tabPageOut' がアクセスされました」
- **原因**: `AnalyzeAsync` が別スレッドで `GetModeFactory()` を呼び、タグ分岐が `tabPageOut.SelectedTab` 等に直接触っていた。週間/SP経路は例外にならないプロパティだけ触っていたため顕在化しなかった
- **対策**: 実行ボタン押下時（UIスレッド）に検索条件・DBパスを専用クラス（`TagExecuteContext`）に退避し、集計スレッド側は退避値のみ使う。`GetModeFactory()` 内ではコントロール参照禁止（週刊/SPの `DateTimePicker.Value` 参照も本則違反のため将来の改修対象）

### 20. 週間差分の7日窓の根拠（SabunReader）

- 事実（2019〜2024-06-15）: 本来は窓を入れたくなかったが、2019年以前のデータがなく完璧な差分補完はできない問題の一つ。週ごとに出入りする動画が多く差分なしがノイズになるため編集部と協議し、「先週の差分なし」で諦めると足切りが多かったことから区切りよく先々週月曜（＝7日）までを窓にした。7に厳密な数字的根拠の記憶なし。`b3a8567`でメンテ日を日数に数えない方式に変更。きっかけは2024-06攻撃・長期メンテ（従来はニコ動のメンテナンスは数時間〜半日で影響考慮不要だった）。先々週丸ごとメンテなら同じ問題になるためメンテ分を除外した
- 判断（2024-06-15／聞き取り2026-09-07・#33）: メンテ日はランキング取得不可＝差分基準になりえないため、日数に数えると窓が実質狭まるので除外する。窓外でも投稿日が窓内なら差分なしで採用・窓より前なら対象外とするのは、古い動画の再浮上より新着動画を優先するため。基準を無計画に変えるのはリスク（#31の1年保持の根拠にもなっている）のため7は変える予定なし
...
- **原因**: `JObject.Parse` の既定（`DateParseHandling.DateTime`）ではISO日時文字列が `Date` トークンに自動変換され、文字列比較の前提が崩れる。`ToString()` した値では `+09:00` のオフセット情報も落ちる
...
### 23. 朝の測定・調査と本取得タスクの競合（Issue #38・2026-09）

- **症状**: 調査用の5分ポーリングで A（version取得）・B（件数取得）が同時失敗し次枠で復帰する枠が散発した（9/21に4枠）
- **原因**: 同日朝のスナップショット本取得（06:37〜08:11・100件ページング4並列の大量リクエスト）と時間帯が重なった。宛先ホストが同一（snapshot.search.nicovideo.jp）のため巻き添えで失敗したと判断する（うち3枠が実行時間帯と一致。残り1枠は別の一過性）
- **対策**: 朝の測定・調査で失敗枠が出たら、まず本取得タスクの実行時間帯と突き合わせる。API異常やスクリプト不具合と誤診しない
- **補足**: 本実装（#38）は `InternetUtil` の20回リトライ＋確認不能時の継続設計のため、この程度の一過性は吸収される
