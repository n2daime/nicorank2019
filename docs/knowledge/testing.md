...
└── nicorankLib/
    ...
    ├── Analyze/Option/Ext/ UnitTestFavoriteTagReader(6)
...
| `UnitTestRankingDisplayTags` | 5 | `GetDisplayTags` のカテゴリ除外・順序・Trim・重複・非破壊（Issue #27） |
| `UnitTestFavoriteTagReader` | 6 | 人気タグ＋ロックタグ全件補完・重複除外・対象外・確保失敗（Issue #27） |
