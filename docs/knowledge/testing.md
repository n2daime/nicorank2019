...
└── nicorankLib/
    ...
    ├── SnapShot/   UnitTestSnapShotRequest(12) / UnitTestTagConditionParser(7) / UnitTestTagSearchRequest(7) / UnitTestSnapShotVersionChecker(11) / UnitTestSnapShotVersionPoller(4)
...
| `UnitTestSnapShotVersionChecker` | 11 | version判定の同日・前日・月またぎ・非JST環境・取得失敗・例外・JSON破損・欠落・パース失敗・ログ文面（Issue #38） |
| `UnitTestSnapShotVersionPoller` | 4 | 待機仕様値の固定・即更新・リトライ後更新・タイムアウト打ち切り（フェイク時計。Issue #38） |
