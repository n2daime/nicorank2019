...
{
    [TestClass]
    public class UnitTestRankRssXml
    {
        [TestMethod]
        public void Test_GetRankRssXml()
        {
            bool isOpened = TextUtil.ReadText("TestRSS.xml", out string strXml);
            if (!isOpened)
            {
                return;
            }
            var testResult1 = RankRssXml.CreateFileNameList();

            var testResult2 =  RankRssXml.CreateRankLogJson(strXml);
